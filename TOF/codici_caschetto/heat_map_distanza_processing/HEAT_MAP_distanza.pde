...
void draw(){
  ...
  // Adjust terrain array size based on cols and rows
  ...
  for(int y=0; y<rows-1; y++){
    ...
    for(int x=0; x<cols; x++){
      ...
      //(DATI, VALORE_MIN, VALORE_MAX, COLORE_MIN(rosso), COLORE_MAX(blu))
...
void serialEvent(Serial p){ 
  ...
  if (buff != "") {
    ...
    // Dynamically adjust cols and rows based on the number of data points
    //if (depths.length == 16) {
    //  cols = 4;
    //  rows = 4;
    //} else if (depths.length == 64) {
    //  cols = 8;
    //  rows = 8;
    //}
  }
