...
int SMA_buffer[matrixSize][matrixSize][SMA_WINDOW_SIZE] = { 0 };
int SMA_velocity[matrixSize][matrixSize]={ 0 };
int SMA_distance[matrixSize][matrixSize]={ 0 };
int SMA_distance_past[matrixSize][matrixSize]={ 0 };
int SMA_distance_past2[matrixSize][matrixSize]={ 0 };
int time_istant=0;  //flag to keep track of the which value we are getting inside the SMA buffer
SoftwareTimer blinkTimer;  //timer used for ISR

float smoothed[matrixSize][matrixSize]; // Matrice smussata

// Kernel Gaussiano 3x3 normalizzato
...
void applyGaussianFilter() {
  ...
  for (int i = 0; i < matrixSize; i++) {
    for (int j = 0; j < matrixSize; j++) {
      float sum = 0.0; // Somma pesata dei valori
      for (int ki = -1; ki <= 1; ki++) { // Itera sul kernel (3x3)
        for (int kj = -1; kj <= 1; kj++) {
          int ni = i + ki; // Indice della riga vicina
          int nj = j + kj; // Indice della colonna vicina
          // Controlla i bordi della matrice
          if (ni >= 0 && ni < matrixSize && nj >= 0 && nj < matrixSize) {
            sum += delta_v[ni][nj] * gaussianKernel[ki + 1][kj + 1];
          }
        }
      }
      // Salva il valore smussato
      smoothed[i][j] = sum;
    }
  }
}
...
void sampling1_total() {
  ...
  memcpy(SMA_distance_past2, SMA_distance_past, sizeof(SMA_distance));
  memcpy(SMA_distance_past, SMA_distance, sizeof(SMA_distance));
  ...
  for (int y = 0; y < matrixSize; y++)  //rows
  {
    for (int x = 0; x < matrixSize; x++)  //column
    {
      ...
      //////////////////////////////////////////////////////////////// SMA VELOCITY ///////////////////////////////////////////////////////
      //populate the buffer until all the N values are filled and then, rewrite the oldest value with the new one
      // SMA_buffer[y][x][time_istant]=delta_v[y][x];
      // if(time_istant<SMA_WINDOW_SIZE)
      // {
      //   //do nothing and wait until the buffer is full
      //   time_istant++;
      // }
      // else
      // {
      //   //start ranging
      //   for(int h=0;h<SMA_WINDOW_SIZE; h++)
      //   {
      //     SMA_velocity[y][x]+= SMA_buffer[y][x][h];
      //   }
        
      //   SMA_velocity[y][x]= SMA_velocity[y][x]/SMA_WINDOW_SIZE;
      //   time_istant= (time_istant+1)%SMA_WINDOW_SIZE;
      // }

      ////////////////////////////////////////////////////////////// SMA DISTANCE /////////////////////////////////////////////////////////
      //populate the buffer until all the N values are filled and then, rewrite the oldest value with the new one
      SMA_buffer[y][x][time_istant]=distanceMatrix[y][x];
      if(time_istant<SMA_WINDOW_SIZE)
      {
        //do nothing and wait until the buffer is full
      ...
      else
      {
        ...
        for(int h=0;h<SMA_WINDOW_SIZE; h++)
        {
          SMA_distance[y][x]+= SMA_buffer[y][x][h];
        }
        
        SMA_distance[y][x]= SMA_distance[y][x]/SMA_WINDOW_SIZE;
        time_istant= (time_istant+1)%SMA_WINDOW_SIZE;
      }
      delta_v[y][x] = (SMA_distance_past2[y][x] - SMA_distance[y][x])/time_ISR;
  ...
  for (int y = 0; y < matrixSize; y++)
  {
    ...
    {
      Serial.print(SMA_distance[y][x]);
      Serial.print(","); //to use processing
      Serial.print(smoothed[y][x]);
      Serial.print(",");
    }
  }
  Serial.println();
