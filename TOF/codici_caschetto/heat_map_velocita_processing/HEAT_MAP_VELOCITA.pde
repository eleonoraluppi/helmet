...
void draw(){
  ...
  colorMode(HSB, 360, 255, 255); // HSB color space makes it easy to map hue
  ...
  for(int y=0; y<rows-1; y++){
    ...
    for(int x=0; x<cols; x++){
      //FILL IMPOSTA IL COLORE DI RIEMPIMENTO DI UNA FORMA
      //fill(map(terrain[x][y],0,150,0,240),255,255);
      //fill(map(Math.abs(terrain[x][y]),0,150,255,0),255,255);
      //(DATI, VALORE_MIN, VALORE_MAX, COLORE_MIN(blu), COLORE_MAX(rosso))
...
// Generate heatmap color based on value
color getHeatmapColor(float value) {
  float hue;
  
  // Gestione esplicita di 0
  if (value == 0) {
    hue = 240; // Blu
  } 
  // Valori positivi
    ...
    if (value <= 75) {
      hue = map(value, 0, 75, 240, 180); // Blu → Ciano
    } else {
      hue = map(value, 75, 150, 180, 0); // Ciano → Rosso
    }
  } 
  // Valori negativi
  else {
    if (value >= -75) {
      hue = map(value, -75, 0, 300, 240); // Viola → Blu
    } else {
      hue = map(value, -150, -75, 0, 300); // Rosso → Viola
    }
  }
  
  return color(hue, 255, 255); // Usa l'intero range di saturazione e luminosità
}
